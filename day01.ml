...
let rec find2sum_rec (sorted: int list) (reversed: int list) (sum: int): (int*int) option =
...
let find3sum input sum = 
  List.find_map input ~f:(fun c -> Option.map (find2sum input (sum - c)) ~f:(fun (a,b) -> (a,b,c)))
...
let () =
  let input = In_channel.read_lines "./input/01.txt" |> List.map ~f:Int.of_string in
