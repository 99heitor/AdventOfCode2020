...
type password_data = { first: int; second: int; letter: char; password: string }
...
let parse_password_data line = 
  ...
  | [dashed; letter_colon; password] -> 
    ...
    Option.map (parse_dashed dashed) ~f:(fun (first,second) -> {first; second; letter; password})
  | _ -> None

let rule_1 data: bool = 
  let letter_count = String.count data.password ~f:(fun lett -> Char.equal data.letter lett) in 
...
let count_valid_passwords ~rule password_list =
  List.map password_list ~f:rule 
  |> List.count ~f:(Bool.equal true)

let () =
  let password_list = In_channel.read_lines "./input/02.txt" |> List.filter_map ~f:parse_password_data in
